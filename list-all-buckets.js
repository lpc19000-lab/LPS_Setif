const admin = require('firebase-admin');

const config = {
    projectId: "lps19-121b2",
    clientEmail: "firebase-adminsdk-fbsvc@lps19-121b2.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\ \n71/PniTEBnjgUjMUUaRoXS1T\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n')
};

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert(config)
    });
}

const storage = admin.storage();

async function listAllBuckets() {
    try {
        // In firebase-admin, getStorage().bucket().storage is the @google-cloud/storage instance
        const sdkStorage = storage.bucket().storage;
        console.log('Fetching all buckets for project:', config.projectId);
        const [buckets] = await sdkStorage.getBuckets();
        console.log('Buckets found:', buckets.map(b => b.name));
    } catch (e) {
        console.log('Error listing buckets:', e.message);
        if (e.message.includes('NOT_FOUND')) {
           console.log('Hint: The project ID might be wrong or the service account does not have access to any buckets.');
        }
    }
    process.exit(0);
}

listAllBuckets();
