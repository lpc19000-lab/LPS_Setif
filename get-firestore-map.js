const admin = require('firebase-admin');
const fs = require('fs');

// Hardcoded for the script using .env.local values
const config = {
    projectId: "lps19-121b2",
    clientEmail: "firebase-adminsdk-fbsvc@lps19-121b2.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n71/PniTEBnjgUjMUUaRoXS1T\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n')
};

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert(config),
        storageBucket: "lps19-121b2.firebasestorage.app"
    });
}

const db = admin.firestore();

async function getData() {
    try {
        const mSnap = await db.collection('manufacturers').get();
        const manufacturers = {};
        mSnap.forEach(doc => {
            manufacturers[doc.data().name.toUpperCase()] = doc.id;
        });
        
        const pSnap = await db.collection('products').get();
        const existingProducts = {};
        pSnap.forEach(doc => {
            existingProducts[doc.data().name.toUpperCase()] = doc.id;
        });
        
        fs.writeFileSync('firestore-data.json', JSON.stringify({ manufacturers, existingProducts }, null, 2));
        console.log(`Retrieved ${Object.keys(manufacturers).length} manufacturers and ${Object.keys(existingProducts).length} products.`);
    } catch (e) {
        console.error(e);
    } finally {
        process.exit(0);
    }
}

getData();
