const admin = require('firebase-admin');

const config = {
    projectId: "lps19-121b2",
    clientEmail: "firebase-adminsdk-fbsvc@lps19-121b2.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\ \n71/PniTEBnjgUjMUUaRoXS1T\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n')
};

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert(config)
    });
}

async function debugStorage() {
    try {
        console.log('Project ID:', admin.app().options.projectId);
        // Correct way to get the underlying storage object in Admin SDK
        const storage = admin.storage();
        // and its listBuckets (it's actually on the storage instance)
        // Note: listBuckets is on the @google-cloud/storage Storage instance
        // available via storage.app.storage() usually? No.
        // It's storage.storage which is the @google-cloud/storage instance.
        // Wait, for older versions it was just admin.storage().storage.
        // Let's try to just get the default bucket and see metadata
        const bucket = storage.bucket();
        console.log('Default Bucket Name:', bucket.name);
        try {
            const [metadata] = await bucket.getMetadata();
            console.log('Default Bucket Metadata:', metadata.name);
        } catch (e) {
            console.log('Default bucket metadata fetch failed:', e.message);
        }
        
    } catch (e) {
        console.error('CRITICAL ERROR:', e);
    } finally {
        process.exit(0);
    }
}

debugStorage();
