...
async function inspectCollections() {
    try {
        ...
        // Check first product to see its manufacturer field
        const pSnap = await db.collection('products').limit(1).get();
        if (!pSnap.empty) {
            console.log('Sample Product:', JSON.stringify(pSnap.docs[0].data(), null, 2));
        }
    } catch (e) {
        console.error(e);
    } finally {
