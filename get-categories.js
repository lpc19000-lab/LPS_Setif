...
async function getCategories() {
    try {
        ...
        const categories = {};
        cSnap.forEach(doc => {
            categories[doc.data().name.toUpperCase()] = doc.id;
        });
        fs.writeFileSync('firestore-categories.json', JSON.stringify(categories, null, 2));
        console.log(`Retrieved ${Object.keys(categories).length} categories.`);
    } catch (e) {
        console.error(e);
    } finally {
        process.exit(0);
